...
import { getLoginHistory } from "../api/loginHistoryApi.js";
  ...
  const loadHistory = async () => {
    try {
      ...
      if (response.success) {
        ...
        setError(response.message || "Failed to load login history");
      }
    } catch (error) {
  ...
  return (
    <div className="login-history-page">
      <div className="login-history-header">
        <div>
          <h1>Login History</h1>
          <p>User login and logout activity</p>
        </div>
              ...
              {history.length === 0 ? (
                ...
                history.map((item, index) => (
                  <tr key={item._id}>
                    ...
                    <td>{formatDateTime(item.logoutTime)}</td>
                    <td>
                      <span
                        className={
                          item.status === "Active" ? "active-status" : "logout-status"
                        }
                      >
                        {item.status}
                      </span>
